...
#
# subset = node_list
# main_set = nodes_dictionary_to_nodes_list(start_dictionary) +
# node_list + nodes_dictionary_to_nodes_list(end_dictionary)
#
#
...
for _ in np.arange(5):
    ...
    result = main('D:/Users/Henri/Documents/Esisar/5A/PFE/Info quantique/Quantomatic/zx-project-1.1/graphs/test.qgraph',
                  ['b0', 'b1'])
    print(result[1])
    expected_result = np.matrix([[0.5, 0, 0, 0.5], [0.5, 0, 0, 0.5]], dtype=complex)
    dist = np.linalg.norm(result[0] - expected_result)
    is_correct = dist < 10 ** (-10)
    print(is_correct)
    # print(result[0])
    first_pass = False
    if is_correct:

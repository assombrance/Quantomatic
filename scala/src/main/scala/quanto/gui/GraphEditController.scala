...
class GraphEditController(view: GraphView, undoStack: UndoStack, val readOnly: Boolean = false) extends ClipboardOwner {
...
//          }
  ...
  view.reactions += {
    ...
    case KeyPressed(_, Key.M, modifiers, _)  =>
      if (modifiers  == 0) {
        mouseState = MatrixTool()
        controlsOpt.map { c => c.setMouseState(mouseState) }
      }
